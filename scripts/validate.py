# ==============================================================================
# Script:           validate.py
# Purpose:          Evaluates model performance on the validation cohort
...
def main():
    args = parse_args()
    log_header(config_path = Path(args.config_dir) / "validation.yaml")

    # Load workflow and path configurations
    config = load_configs(
        pipeline = Path(args.config_dir) / "validation.yaml",
...
def log_header(config_path):
    ...
    logger.info("- Pipeline Stage: Model Validation")
